...
const DeviceDetails = ({ barcode, state }) => {
  ...
  return (
    <>
      <SkipChangesDialog handleSkipChanges={handleSaveDevice} isOpen={isOpen} setIsOpen={setIsOpen} />
      <Box border="1px" borderColor="gray.300" borderRadius="md" mt={8} p={4}>
        <Heading>Datos Equipo</Heading>
        <Divider orientation="horizontal" />
        <HStack justify="space-between" mt={4}>
          <HStack>
            <Circle bg={getStateColor(device.state)} color="white" size="15px" />
            <Badge fontSize="0.8em" mb={4} variant="solid">
              EQUIPO: {barcode}
            </Badge>
          </HStack>
        ...
        <SimpleGrid columns={3} spacing={10}>
          ...
          <FormControl id="catalogue" mb={4}>
            ...
            <Input isDisabled name="catalogue" value={device?.catalogue || ``} />
          </FormControl>
          <FormControl id="code" mb={4}>
            <FormLabel as="legend">Código Fabricante</FormLabel>
            <Input isDisabled name="code" value={device?.code || ``} variant="filled" onChange={handleInputChange} />
          </FormControl>
        </SimpleGrid>
        ...
        <FormControl id="location" mb={4}>
          ...
          <Select name="location" value={device?.location || 'CIME'} variant="outline" onChange={handleInputChange}>
        ...
        <FormControl id="fleet" mb={4}>
          ...
          <Select name="fleet" value={device?.fleet || ``} variant="outline" onChange={handleInputChange}>
      ...
      {device.repairs?.length > 0 ? (
        <Box border="1px" borderColor="gray.300" borderRadius="md" mt={8} p={4}>
          ...
          {device.repairs.map(
            (repair) =>
              repair.active && (
                <Box key={repair.id} borderColor="teal.600" borderRadius="lg" borderWidth="1px" mt={4} p={4}>
          ...
          {device.repairs[device.repairs.length - 1].exitDate ? (
            <Tooltip label="Crear nuevo ingreso al Laboratorio" placement="right-start">
              <IconButton colorScheme="teal" mt={4} variant="outline" onClick={() => handleNewRepair(device.id)}>
                <FaPlus />
              </IconButton>
            </Tooltip>
          ) : null}
